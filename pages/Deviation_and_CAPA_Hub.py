...
with col1:
    ...
    if not oos_devs.empty:
        ...
        fig_rc = px.bar(
            ...
            y="Root Cause",
            orientation='h',
            title="OOS Investigations by Root Cause",
            color="Count",
            color_continuous_scale=px.colors.sequential.Reds
        )
        # Sort y-axis by the total count to create the Pareto effect.
...
with col2:
    ...
    # This chart was already well-implemented and uses data from the session state.
    ...
    if not closed_devs.empty:
        fig_cycle = px.histogram(
            ...
            x="Age_Days",
            nbins=10,
            title="Cycle Time for Closed Deviations",
...
with reg_col1:
    ...
    # Set sensible default dates for the date_input widget.
...
with reg_col2:
    if st.button("Generate Regulatory Data Summary"):
        # Ensure the date range is valid (st.date_input always returns a tuple of length 2).
        if len(date_range_reg) == 2:
            start_date, end_date = date_range_reg
            
            # OPTIMIZATION: Perform date filtering on the entire datetime object, which is faster.
            # Pandas can use optimized vectorized operations on datetime columns.
            # Filtering on `.dt.date` is slower as it requires a per-row conversion.
            start_ts = pd.to_datetime(start_date)
            end_ts = pd.to_datetime(end_date) + pd.Timedelta(days=1) # Include the whole end day
            ...
            # This case is unlikely to be triggered with st.date_input but is good practice.
