...
# Filter data for the selected partner
...
with tab_perf:
    ...
    with col1:
        ...
        # Simulate Cpk data
        usl, lsl = 99.8, 98.0
        mu, sigma = np.random.uniform(98.5, 99.5), np.random.uniform(0.1, 0.4) # Make it dynamic per partner
...
with tab_ml:
    ...
    st.markdown("""
    - **What:** An unsupervised ML model (**Isolation Forest**) trained on historical results for a key stability-indicating assay. It learns the "normal" multivariate distribution of results (e.g., the relationship between Purity and the Main Impurity).
    ...
    - **Why (Actionability):** This is a powerful tool for **proactive risk identification**. An anomalous result, even if in-spec, can be a leading indicator of method drift, instrument issues, or subtle changes in product quality. It provides a data-driven reason to place a batch on "for-information" stability or perform additional characterization, aligning with the principles of **ICH Q10 (Pharmaceutical Quality System)**.
    """)
    np.random.seed(0)
    # Simulate historical data for the model to learn from
...
with tab_transfers:
    ...
    st.markdown("This tracker monitors the progress of all active analytical method transfers to this partner, ensuring clear communication with the Analytical Development team and preventing delays in bringing new testing capabilities online.")
    tt_data = [
