    ...
    if st.button("Generate QBR PowerPoint Report"):
        with st.spinner("Creating QBR Deck..."):
            ...
            try:
                ...
                elif not partner_batches.empty: # Re-generate if not in scope
                    sla = partner_batches['TAT_SLA'].iloc[0]
                    fig_hist_local = px.histogram(partner_batches, x="Actual_TAT", nbins=15, title=f"Testing TAT Distribution (SLA: {sla} days)", color_discrete_sequence=['#955196'])
                    fig_hist_local.add_vline(x=sla, line_dash="dash", line_color="red", annotation_text="SLA")
                    hist_img_bytes = fig_hist_local.to_image(format="png", width=800, height=400)
                    slide.shapes.add_picture(io.BytesIO(hist_img_bytes), Inches(5.0), Inches(1.5), width=Inches(4.5))
            ...
            st.download_button(
                ...
                data=ppt_buffer,
                file_name=f"QBR_{selected_partner.replace(' ', '_')}_{pd.Timestamp('2023-10-27').strftime('%Y%m%d')}.pptx",
